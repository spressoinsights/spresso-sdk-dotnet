...
namespace SpressoAI.MockApi.Controllers;
...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> GetToken([FromBody] OAuthTokenRequest request, CancellationToken cancellationToken)
    {
        ...
        claims.Add(new Claim("https://api.spresso.com/services", JsonSerializer.Serialize(serviceList),
            JsonClaimValueTypes.JsonArray));
        claims.Add(new Claim("gty", "client-credentials"));
        claims.Add(new Claim("permissions",
            JsonSerializer.Serialize(claims.Single(c => c.Type == "scope").Value.Split(' ')),
            JsonClaimValueTypes.JsonArray));
        claims.Add(new Claim("azp", request.client_id!));
        claims.Add(new Claim("sub", request.client_id + "@clients"));
        claims.Add(new Claim("iss", "https://mock-spresso-auth"));


        var jwtPayload = new JwtPayload("https://mock-spresso-auth", "https://mock-spresso-api", claims,
            DateTime.UtcNow,
        ...
        var jwt =
            new JwtSecurityToken(
                new JwtHeader(new SigningCredentials(
                    new SymmetricSecurityKey(Encoding.UTF8.GetBytes("NcQfTjWnZr4u7x!A")),
